...
def test_approve(token, accounts):
    token.approve(accounts[1], 10**20, {'from': accounts[0]})

    assert token.allowance(accounts[0], accounts[1]) == 10**20
...
def test_approve_self(token, accounts):
    token.approve(accounts[0], 10**20, {'from': accounts[0]})

    assert token.allowance(accounts[0], accounts[0]) == 10**20
...
def test_returns_true(token, accounts):
    tx = token.approve(accounts[1], 10**20, {'from': accounts[0]})
...
def test_approval_event_fires(accounts, token):
    ...
    assert tx.events["Approval"].values() == [accounts[0], accounts[1], 10**20]
